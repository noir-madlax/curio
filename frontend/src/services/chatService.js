...
export const startSurveyChat = async (responseId) => {
  ...
  const apiUrl = `${import.meta.env.VITE_API_BASE_URL}/api/v1/survey-conversations/first_chat`;
...
export const sendSurveyMessage = async (responseId, message) => {
  ...
  // 2024-04-26: 使用环境变量，不硬编码API地址
  const apiUrl = `${import.meta.env.VITE_API_BASE_URL}/api/v1/survey-conversations/chat`;
